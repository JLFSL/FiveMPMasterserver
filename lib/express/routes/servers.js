...
app.put('/v2/servers', function(req, res) {
  if (!(req.body.ip) || !(req.body.port)) {
    return res.json({ message: 'Unprocessable', error: 'Unprocessable Entity', status: 422 });
  }
  ...
  SM.findOne({ 'ip': req.body.ip, 'port': req.body.port }, function(err, result) {
    ...
    result.blacklisted = req.body.blacklisted;
    result.stats.verified = req.body.verified;
    result.updated_at = util.getISOStamp();

    //
    // console.log(req.body.blacklisted);
    // result.blacklisted = (req.body.blacklisted) ? ((req.body.blacklisted == true) ? true:false):result.blacklisted;
    // console.log(result.blacklisted);

    // result.blacklisted = (req.body.blacklisted) ? (req.body.blacklisted == true) ? true:false:result.blacklisted,
    // result.verified = (req.body.verified) ? (req.body.verified == true) ? true:false:result.verified,
...
app.post('/v2/servers', function(req, res) {
  ...
  if (!(content.port) || !(content.name) || (content.players.amount == null) || (content.players.max == null)) {
    return res.json({ message: 'Unprocessable', error: 'Unprocessable Entity', status: 422 });
  }
